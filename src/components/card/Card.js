...
export default function Card () {
  ...
  return (
    items.map(item => (
      ...
      <div className="card-container" key={item.product.id}>
        <div className="imgs-container">
          <button>
            <img className="image" src={item.product.images[0]} alt="Logo"/>
          </button>
        </div>
        ...
        <div className="description-container">
          ...
          <hr/>
          <p className="good-price">MELHOR PREÇO</p>
          <div className="price-container">
            <p className="divide-times">{item.product.price.installments}R$</p>
            <p className="price-divide">{item.product.price.installmentValue}</p>
            <button className="button">Adicionar ao carrinho ></button>
          </div>

          <div className="full-price-container">
            <p>ou </p>
            <p className="full-price">{item.product.price.value}</p>
            <p>à vista</p>
          </div>
        </div>
      </div>
